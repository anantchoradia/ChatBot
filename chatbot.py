...
c=pd.DataFrame(answers)
print(c)
d=pd.DataFrame(questions)
print(d)
...
a=pd.DataFrame(clean_answers)
print(a)
b=pd.DataFrame(clean_questions)
print(b)

# remove non  frequnt words
word2count={}
...
d=pd.DataFrame.from_records(word2count,index=[0])
...
for word,count in word2count.items():
	if count >= threshold: 
		...
		word_number+=1
s=pd.DataFrame.from_records(answerswords2int,index=[0])
print(s)		
...
x=pd.DataFrame.from_records(answersint2word,index=[0])
print(x)		
...
aa=pd.DataFrame(answers_into_int)
print(aa)
bb=pd.DataFrame(questions_to_int)
print(bb)  
...
def decode_training_set(encoder_state, decoder_cell , decoder_embedded_input,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
	...
	training_decoder_funtion = tf.contrib.seq2seq.attention_fn_train(encoder_state[0],attention_keys,attention_values,attention_score_function,attention_construct_function,name='attn_dec_train')
...
with tf.name_scope("optimization"):
	loss_error=tf.contrib.seq2seq.sequence_loss(training_predictions,targets,tf.ones([input_shape[0],sequence_loss]))

	optimizer=tf.train.AdamOptimizer(learning_rate)
	gradients=optimizer.compure_gradients(loss_error)
...
def split_into_batches(questions,answers,batch_size):
	for batch_index in range(0,len(questions)//batch_size):
		...
		padding_questions_in_batch=np.array(apply_padding(questions_in_batch,questionswords2int))
		padding_answers_in_batch=np.array(apply_padding(answers_in_batch,answerswords2int))
		yield padding_answers_in_batch,padding_questions_in_batch
...
for epoch in range(1,epochs+1):
	for batch_index,(padded_questions_in_batch,padded_answers_in_batch) in enumerate(split_into_batches(training_questions,training_answers,batch_size)):
		...
		if batch_index%batch_index_check_training_loss==0:
			print("Epoch: {:>3}/{},Batch:{:>4}/{},Training Loss Error: {:>6.3f},Trainin Time on 100 Batches:{:d} seconds".format(epoch,epochs,batch_index,len(training_questions)//batch_size,total_training_loss_error/batch_index_check_training_loss,int(batch_time*batch_index_check_training_loss)))
			total_training_loss_error=0
		if batch_index %batch_index_check_training_loss==0 and batch_index>0 :
			...
			print("Validation Loss Error:{:>6.4f}, Batch Validation Time: {:d} seconds".format(average_validation_loss_error,int(batch_time)))
